...
global_match <- j24$twa > 105 #j24$time_to_mark < 100000

plot(j24$twa[global_match],
     j24$time_to_mark[global_match],
     type="n",
     xlab="Wind Angle",
     main="J24 Time To Mark, 1nm, Given Polars",
...
for (i in 1:length(unique(j24$tws))) {
  ...
  # Put a point of where the boat is fastest.
  # Commented out because it's usally like 90 degrees.
  # points(
  #   x_data_twa[min_bs],
  #   y_data_ttm[min_bs],
  #   pch=3
...
## Plot and model the sigmoid curve 
if (FALSE){
  df = data.frame(
    tws = unique(j24$tws),
    twa = min_twas
  ...
  plot(df$tws, df$twa)#, xlim=c(0,20), ylim=c(100,180))
  
  
  #mo = lm(twa ~  tws, data=df)# linear
  mo = lm(twa ~ ns(tws, 4), data=df )# split
  lines(seq(0, max(df$tws)), predict(mo, list(tws = seq(0,  max(df$tws))))   )
